let handler = async (m, { usedPrefix, command }) => {
  ...
  if (/video/i.test(command))
    fltr = split
      ...
      .map((v) => "" + v.nama)
      .join("\n");
  if (/gif/i.test(command))
    fltr = split
      .filter((v) => v.message.videoMessage)
      .filter((m) => m.message.videoMessage.gifPlayback)
      .map((v) => "" + v.nama)
      .join("\n");
  if (/stic?ker/i.test(command))
    fltr = split
      .filter((v) => v.message.stickerMessage)
      .map((v) => "" + v.nama)
      .join("\n");
  if (/msg/i.test(command))
    fltr = split
      .filter((v) => v.message.conversation)
      .map((v) => "" + v.nama)
      .join("\n");
  if (/img/i.test(command))
    fltr = split
...
handler.help = ["vn", "msg", "video", "gif", "audio", "img", "sticker"].map(
  (v) => "list" + v
);
